...
Módulo 1 — BACKTEST OUT-OF-SAMPLE
  - Split 75/25: treina nos primeiros 3 anos (~756 pregões),
    avalia no último ano (~252 pregões) — experimento 4 anos
...
DISCLAIMERS:
  ...
  • Backtest 75/25 (split único) é ilustrativo; robustez avaliada
    pelo bootstrap (MIN_TREINO=750) em monte_carlo.py.
...
OUT_PATH = os.path.join(PASTA, "planilhas", "backtest_benchmark.xlsx")
...
N_TOTAL = len(df_all)
SPLIT   = int(N_TOTAL * 0.75)  # 75/25 — ~756 treino / ~252 teste
...
print(f"Treino (in-sample) : {df_train.index[0].date()} → {df_train.index[-1].date()} ({len(df_train)} pregões — 75%)")
print(f"Teste (out-of-sample): {df_test.index[0].date()} → {df_test.index[-1].date()} ({len(df_test)} pregões — 25%)")
